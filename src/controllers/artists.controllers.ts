...
export const getAllArtists = async (req: Request, res: Response) => {
    try {
      ...
      res.status(201).send({ msg: 'Here are all your artists', data: allArtists });
    } catch (error) {
  ...
  export const createArtist = async (req: Request, res: Response) => {
    ...
    try {
      ...
      export const deleteArtist = async (req: Request, res: Response) => {
        ...
        try {
          const artistDeleted = await prisma.artist.delete({ where: { id: userId } });
          ...
          res
            .status(201)
            .send({ msg: 'Artist has been deleted successfully', data: artistDeleted });
        } catch (error) {
          res.status(400).send({ msg: 'ERROR' });
        }
      };
